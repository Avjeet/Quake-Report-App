    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                Formatter timeDate = new Formatter(properties.getLong("time"));
                Formatter loc = new Formatter(properties.getString("place"));
                Formatter mag = new Formatter((properties.getDouble("mag")));

                earthquakes.add(new Earthquake(loc.getLocationoffset(),loc.getLocation(), mag.getMagnitude(), mag.magnitudeDouble,timeDate.getDate(), timeDate.getTime(),properties.getString("url")));
            }
...
class Formatter {
    ...
    Formatter(String s) {
        String[] loc = s.split("(?<=of )",2);
        mLocationOffset=loc[0];
        mLocation=loc[1];
    }
    Formatter(Double d){
